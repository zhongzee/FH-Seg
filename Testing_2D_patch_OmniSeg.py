...
def get_arguments():
    ...
    parser.add_argument("--reload_path", type=str, default='snapshots_2D/final/Omniseg/MOTS_DynConv_fold1_with_white_scale_normalwhole_1217_e160.pth')
    ...
    parser.add_argument("--best_epoch", type=int, default=160)
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        with torch.no_grad():
            for iter, batch in enumerate(valloader):
                ...
                output_folder = os.path.join('./DoDNet/MIDL/MIDL_github/testing_%s' % (args.validsetname), str(args.best_epoch))
                ...
                if task6_pool_image.num_imgs >= batch_size:
                    ...
                    preds, _ = model(images, torch.ones(batch_size).cuda() * 6, scales)
            ...
            if (task2_pool_image.num_imgs < batch_size) & (task2_pool_image.num_imgs >0):
                ...
                preds, _ = model(images, torch.ones(left_size).cuda() * 2, scales)
                now_task = torch.tensor(2)
            ...
            if (task3_pool_image.num_imgs < batch_size) & (task3_pool_image.num_imgs >0):
                ...
                preds, _ = model(images, torch.ones(left_size).cuda() * 3, scales)
                now_task = torch.tensor(3)
            ...
            if (task4_pool_image.num_imgs < batch_size) & (task4_pool_image.num_imgs >0):
                ...
                preds, _ = model(images, torch.ones(left_size).cuda() * 4, scales)
                now_task = torch.tensor(4)
            ...
            if (task5_pool_image.num_imgs < batch_size) & (task5_pool_image.num_imgs >0):
                ...
                preds, _ = model(images, torch.ones(left_size).cuda() * 5, scales)
                now_task = torch.tensor(5)
            ...
            if (task6_pool_image.num_imgs < batch_size) & (task6_pool_image.num_imgs >0):
                ...
                preds, _ = model(images, torch.ones(left_size).cuda() * 6, scales)
                now_task = torch.tensor(6)
                # print("PREDS.shape:", preds.shape)
        ...
        ##########################################################################
        df.loc[6] = ['5ptc', avg_val_F1[6].item(), avg_val_Dice[6].item(), avg_val_HD[6].item(), avg_val_MSD[6].item()]
        ##########################################################################
