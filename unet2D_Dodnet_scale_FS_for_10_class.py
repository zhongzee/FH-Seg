...
class unet2D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        self.conv1s = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=1, padding=(1, 1), dilation=1, bias=False)
        ...
        ################################################################################################
        self.controller_trilinear = nn.Conv2d(256 * 10 * 1, 162, kernel_size=1, stride=1, padding=0)
        ###############################################################################################
        ################################################################################################
        #in_ch = (3, 4)
    ...
    def encoding_task(self, task_id):
        ...
        task_encoding = torch.zeros(size=(N, 10))
        for i in range(N):
    ...
    #     ####################################################################
    #     x_cond = torch.zeros((len(scale_encoding), 256 * 10 * 4, 1, 1)).cuda().float()  ####change channel
    #     ####################################################################
...
#####################################################################################
    def forward(self, query_images, support_images, support_labels, task_id, scale_id):
        #task_id = 0
        #scale_id = 0
        ...
        #"B C H W -> B 1 C H W"
        target = torch.unsqueeze(query_images, dim=1)

        support = support_images

        support_mx = torch.cat([support_images, support_labels], dim=2)
        pass_through = []
        ...
        target_cond = torch.zeros((len(scale_encoding), 256 * 10 * 4, 1, 1)).cuda().float()
        ####################################################################
        # for xi in range(len(x_feat)):
        #     now_x_feat = x_feat[xi].squeeze(-1).squeeze(-1)
        #     now_task_encoding = task_encoding[xi].squeeze(-1).squeeze(-1)
