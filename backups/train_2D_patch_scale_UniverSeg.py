...
def get_arguments():
	...
	parser.add_argument("--trainset_dir", type=str, default='/Data/HATs/Data_missing/train/data_list.csv')

	# parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_validationset_patch/data_list.csv')
	parser.add_argument("--valset_dir", type=str, default='/Data/HATs/Data_missing/val/data_list.csv')
	...
	parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/UniverSeg_8class')
				...
				with torch.no_grad():
					for iter, batch1 in enumerate(valloader):
						...
						if task2_pool_image.num_imgs >= batch_size:
							...
							# preds = model(images[:,:,256:768,256:768], torch.ones(batch_size).cuda()*2, scales)

							preds = torch.zeros((batch_size, 2, 512, 512)).cuda()
							for pi in range(batch_size):
								now_img = images[None, pi, :, 256:768,256:768]
								now_spt_img = torch.cat([images[None, 0:pi, :, 256:768,256:768],images[None, pi + 1:, :, 256:768,256:768]], 1)
								now_spt_lbl = torch.cat([labels[None, 0:pi, None, 256:768,256:768],labels[None, pi + 1:, None, 256:768,256:768]],1)
						...
						if task3_pool_image.num_imgs >= batch_size:
							...
							# preds = model(images[:, :, 256:768, 256:768], torch.ones(batch_size).cuda() * 3, scales)
							preds = torch.zeros((batch_size, 2, 512, 512)).cuda()
							for pi in range(batch_size):
								now_img = images[None, pi, :, 256:768,256:768]
								now_spt_img = torch.cat([images[None, 0:pi, :, 256:768,256:768],images[None, pi + 1:, :, 256:768,256:768]], 1)
						...
						if task4_pool_image.num_imgs >= batch_size:
							...
							# preds = model(images[:,:,256:768,256:768], torch.ones(batch_size).cuda()*4, scales)
							preds = torch.zeros((batch_size, 2, 512, 512)).cuda()
							for pi in range(batch_size):
								now_img = images[None, pi, :, 256:768,256:768]
								now_spt_img = torch.cat([images[None, 0:pi, :, 256:768,256:768],images[None, pi + 1:, :, 256:768,256:768]], 1)
