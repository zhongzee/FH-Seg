...
class unet2D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        ################################################################################################
        self.controller_trilinear = nn.Conv2d(256 * 1 * 4, 162, kernel_size=1, stride=1, padding=0)
        ###############################################################################################
    ...
    def encoding_task(self, task_id):
        ...
        task_encoding = torch.zeros(size=(N, 1))
        for i in range(N):
    ...
    def forward(self, input, task_id, scale_id):
        ...
        ####################################################################
        x_cond = torch.zeros((len(scale_encoding), 256 * 1 * 4, 1, 1)).cuda().float()
        ####################################################################
