...
class unet2D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=1, padding=(1,1),dilation=1, bias=False)

        self.layer0 = self._make_layer(NoBottleneck, 64, 64, layers[0], stride=(1, 1))
        self.layer1 = self._make_layer(NoBottleneck, 64, 64, layers[1], stride=(2, 2))
        ...
        self.layer4 = self._make_layer(NoBottleneck, 256, 512, layers[4], stride=(2, 2))

        self.fusionConv = nn.Sequential(
            nn.GroupNorm(16, 512),
            nn.ReLU(inplace=in_place),
            #conv3x3x3(256, 256, kernel_size=(1, 1, 1), padding=(0, 0, 0), weight_std=self.weight_std)
            nn.Conv2d(512, 512, kernel_size=(1, 1), stride=1, padding=(0,0),dilation=1, bias=False)
        )

        self.upsamplex2 = nn.Upsample(scale_factor=(2, 2))
        self.upsamplex4 = nn.Upsample(scale_factor=(4, 4))

        self.x16_resb = self._make_layer(NoBottleneck, 512, 256, 1, stride=(1, 1))
        self.x8_resb = self._make_layer(NoBottleneck, 512, 128, 1, stride=(1, 1))
        self.x4_resb = self._make_layer(NoBottleneck, 256, 32, 1, stride=(1, 1))
        self.x2_resb = self._make_layer(NoBottleneck, 96, 32, 1, stride=(1, 1))
        self.x1_resb = self._make_layer(NoBottleneck, 96, 16, 1, stride=(1, 1))

        self.conv0 = nn.Conv2d(16, 2, kernel_size=(3, 3), stride=1, padding=(1,1),dilation=1, bias=False)
