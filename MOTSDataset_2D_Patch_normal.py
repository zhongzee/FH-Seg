            ...
            for si in range(len(stain_folders)):
                ...
                for ri in range(len(images)):
                    if 'mask' in images[ri]:
                        ...
                        #############################################
                        """
                        print("stain_folders.shape:", len(stain_folders))
                        print("si:", si)
                        print("len(images):", len(images))
                        print("ri:", ri)
                        print("os.path.join:", os.path.join(stain_folders[si], os.path.basename(image_root).replace('_img.png', '_mask.png')))
                        print("glob.glob:", glob.glob(os.path.join(stain_folders[si], os.path.basename(image_root).replace('_img.png', '_mask.png'))))
                        print("os.path.join:",
                              os.path.join(stain_folders[si], os.path.basename(image_root).replace(ext, '_mask.png')))
...
#            self.img_ids = self.img_ids * int(np.ceil(float(max_iters) / len(self.img_ids)))
        ...
        print('Train {} images are loaded!'.format(len(image_path_list)))
        print("训练集前5条=", image_path_list[:5])
    ...
    def __getitem__(self, index):
        ...
        image = resize(image, [3, self.size[0], self.size[0]])
...
class MOTSValDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(256, 256), mean=(128, 128, 128), scale=False,
        ...
        for i in range(len(image_path_list)):
            ...
            # else:
            #name = osp.basename(label_path)
            name = label_path.replace('/','-')# osp.basename(label_path)
            img_file = image_path
            label_file = label_path
        ...
        print("打印验证集信息前5条")
        for i in range(min(5,len(image_path_list))):
            print(image_path_list[i] + ', ' + label_path_list[i] )
