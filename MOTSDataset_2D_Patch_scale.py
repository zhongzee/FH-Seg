    ...
    def __getitem__(self, index):
        ...
        if scale_id == 3:
            ...
            if n > 0.5:
                ...
                image_downsample = resize(image, (length, length, 3),anti_aliasing=False)
                label_downsample = resize(label, (length, length, 3),anti_aliasing=False)

                image_cc = np.zeros(image.shape)
                label_cc = np.zeros(label.shape)
            ...
            if n > 0.66:
                ...
                length_y = int(np.random.rand(1) * length_max)
                cnt = 0
                while label[length_x:length_x + length, length_y:length_y + length].sum() == 0 and cnt <= 100:
                    cnt += 1
                    length_max = image.shape[0] - length
                    length_x = int(np.random.rand(1) * length_max)
                    length_y = int(np.random.rand(1) * length_max)

                image_cc = image[length_x:length_x+length,length_y:length_y+length,:]
                label_cc = label[length_x:length_x+length,length_y:length_y+length,:]
            ...
            for si in range(len(stain_folders)):
                ...
                for ri in range(len(images)):
                    if 'mask' in images[ri]:
                        ...
                        _, ext = os.path.splitext(images[ri])
                        mask_root = \
                        glob.glob(os.path.join(stain_folders[si], os.path.basename(image_root).replace(ext, '_mask*')))[
                            0]
                        task_list.append(int(tasks_id))
                        scale_list.append(int(scale_id))
                        image_path_list.append(image_root)
