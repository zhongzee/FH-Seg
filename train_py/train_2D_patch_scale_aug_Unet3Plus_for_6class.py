...
def get_arguments():
    ...
    parser.add_argument("--exname", type=str, default='train_2D_patch_scale_aug_Unet3Plus_for_6class') #日志存储路径名
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/train_2D_patch_scale_aug_Unet3Plus_for_6class_omniseg-sampled_0723/')
        ...
        for epoch in range(0,args.num_epochs):
            ...
            for iter, batch in enumerate(trainloader):
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    labels = task0_pool_mask.query(batch_size) # torch.Size([1, 512, 512])
                    ...
                    preds = model(images) # torch.Size([batch_size, 6, 512, 512])
                    #print(preds.shape)
                    ##################################################################################
                    #preds = preds[0]
                    ##################################################################################
                ...
                if task1_pool_image.num_imgs >= batch_size:
                    ...
                    ##################################################################################
                    #preds = preds[0]
                    preds = model(images) # torch.Size([4, 6, 512, 512])
                    ##################################################################################
            ...
            base_directory = '/root/autodl-tmp/Omni-Seg_revision/experiments'
            sub_directory = 'train_2D_patch_scale_aug_Unet_for_6class/visualize/Loss'  # 根据需要更改这个子目录
            ...
            # 假设 epoch 是当前的 epoch 索引
            save_learning_rate_or_loss_data(epoch, lr_changes, base_directory, sub_directory, file_prefix, chart_title,
                                            y_label='TrainLoss', legend_label='TrainLoss')
            ###############################################################################################
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        # output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/fold1_with_white',  'experiments/DoDNet/MIDL/Unet6class/HC_validation'), str(epoch))

                        output_folder = os.path.join(
                            '/root/autodl-tmp/Omni-Seg_revision/experiments/'+args.exname+'/validation_noscale_0724',
                            str(epoch))
                    ...
                    # 保存整个训练过程的数据到单个CSV文件
                output_folder = '/root/autodl-tmp/Omni-Seg_revision/experiments/'+args.exname+'/validation_noscale_0724/'
