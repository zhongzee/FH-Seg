...
#sys.path.append("..")
sys.path.append("/Data/TransSeg/UniverSeg")
import glob
import torch
...
###################################################################################
# from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSDataSet as MOTSDataSet
# #from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSValDataSet as MOTSValDataSet
# #from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSValDataSet_512 as MOTSValDataSet
# from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSValDataSet_512_center as MOTSValDataSet
from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSDataSet as MOTSDataSet
from MOTSDataset_2D_Patch_supervise_normal_csv_512 import MOTSValDataSet_512_center as MOTSValDataSet
#######################################################################################

# from MOTSDataset_2D_Patch_normal import MOTSDataSet as MOTSDataSet
# from MOTSDataset_2D_Patch_normal import MOTSValDataSet
########################################################################################
import random
import timeit
...
# from universeg2 import universeg
...
def get_arguments():
	...
	#parser.add_argument("--trainset_dir", type=str, default='./data/Mice_Patches/train/data_list.csv')
	#parser.add_argument("--trainset_dir", type=str, default='./data/Mice_Patches/ddd2/data_list.csv')
	...
	# parser.add_argument("--valset_dir", type=str, default='./data/Human_Patches/small_val/data_list.csv')

	parser.add_argument("--valset_dir", type=str, default='./data/omniseg-sampled/val')
	#parser.add_argument("--valset_dir", type=str, default='./data/Mice_Patches/ddd2/data_list.csv')
	...
	parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/UniverSeg_10class_TM&VH_single')
...
def get_one_hot_prediction(out):
	norm_out = (out - out.min()) / (out.max() - out.min())
	return torch.cat([norm_out, 1-norm_out], 1)[0]

def supervise_learning(images, labels, batch_size, scales, model, now_task, weight, loss_seg_DICE, loss_seg_CE):

	preds = torch.zeros((images.shape[0], 2, 512, 512)).cuda()
	for pi in range(images.shape[0]):
		now_img = images[None, pi, :]
		now_spt_img = torch.cat([images[None, 0:pi, :],images[None, pi + 1:, :]], 1)
		now_spt_lbl = torch.cat([labels[None, 0:pi, None],labels[None, pi + 1:, None]], 1)

		out = model(now_img, now_spt_img, now_spt_lbl)
		preds[pi, :] = out #get_one_hot_prediction(out)
...
def main():
	...
	with Engine(custom_parser=parser) as engine:
		...
		criterion = None
	   # model.image
		#model['efficientsam_ti'] = build_efficient_sam_vitt()

		#with zipfile.ZipFile("/Data/HATs/EfficientSAM-main/weights/efficient_sam_vits.pt.zip", 'r') as zip_ref:
		#    zip_ref.extractall("weights")

		model = universeg(pretrained=False)

		# for param in model.sam.image_encoder.prompt_generator.parameters():
		...
		for epoch in range(0,args.num_epochs):
			...
			task0_pool_image = ImagePool(batch_size * 2)
			task0_pool_mask = ImagePool(batch_size * 2)
			task0_pool_weight = ImagePool(batch_size * 2)
			task0_scale = []
			#task0_layer = []

			task1_pool_image = ImagePool(batch_size * 2)
			task1_pool_mask = ImagePool(batch_size * 2)
			task1_pool_weight = ImagePool(batch_size * 2)
			task1_scale = []
			#task1_layer = []

			task2_pool_image = ImagePool(batch_size * 2)
			task2_pool_mask = ImagePool(batch_size * 2)
			task2_pool_weight = ImagePool(batch_size * 2)
			task2_scale = []
			#task2_layer = []

			task3_pool_image = ImagePool(batch_size * 2)
			task3_pool_mask = ImagePool(batch_size * 2)
			task3_pool_weight = ImagePool(batch_size * 2)
			task3_scale = []
			#task3_layer = []

			task4_pool_image = ImagePool(batch_size * 2)
			task4_pool_mask = ImagePool(batch_size * 2)
			task4_pool_weight = ImagePool(batch_size * 2)
			task4_scale = []
			#task4_layer = []

			task5_pool_image = ImagePool(batch_size * 2)
			task5_pool_mask = ImagePool(batch_size * 2)
			task5_pool_weight = ImagePool(batch_size * 2)
			task5_scale = []
			#task5_layer = []

			task6_pool_image = ImagePool(batch_size * 2)
			task6_pool_mask = ImagePool(batch_size * 2)
			task6_pool_weight = ImagePool(batch_size * 2)
			task6_scale = []
			#task6_layer = []

			task7_pool_image = ImagePool(batch_size * 2)
			task7_pool_mask = ImagePool(batch_size * 2)
			task7_pool_weight = ImagePool(batch_size * 2)
			task7_scale = []
			#task7_layer = []


			##################################################
			task8_pool_image = ImagePool(batch_size * 2)
			task8_pool_mask = ImagePool(batch_size * 2)
			task8_pool_weight = ImagePool(batch_size * 2)
			task8_scale = []
			#task8_layer = []

			task9_pool_image = ImagePool(batch_size * 2)
			task9_pool_mask = ImagePool(batch_size * 2)
			task9_pool_weight = ImagePool(batch_size * 2)
			task9_scale = []
			#task9_layer = []
			...
			if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
				...
				with torch.no_grad():
					for iter, batch1 in enumerate(valloader):
						...
						#output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/', '/data2/DoDNet/MIDL/Universeg_v2_mice_10class_for_old/Mice_validation'), str(epoch))
						output_folder = os.path.join('/data2/DoDNet/Universeg_mice_10class_TM&VH_single/Mice_validation', str(epoch))
						...
						# 				now_img = images[None, pi, :, now_x:now_x + patch_size, now_y:now_y + patch_size]
						# 				now_spt_img = torch.cat([images[None, 0:pi, :, now_x:now_x + patch_size, now_y:now_y + patch_size],images[None, pi + 1:, :, now_x:now_x + patch_size,
						# 					 now_y:now_y + patch_size]], 1)
						# 				now_spt_lbl = torch.cat([labels[None, 0:pi, None, now_x:now_x + patch_size,now_y:now_y + patch_size],labels[None, pi + 1:, None, now_x:now_x + patch_size,
						# 										 now_y:now_y + patch_size]], 1)
						#
						# 				out = model(now_img, now_spt_img, now_spt_lbl)
						# 				preds[pi, :, now_x:now_x + patch_size,
						# 				now_y:now_y + patch_size] = out
						#
						...
						if task0_pool_image.num_imgs >= batch_size:
							...
							preds = torch.zeros((batch_size, 2, 512, 512)).cuda()
							for pi in range(batch_size):
								now_img = images[None, pi, :]
								now_spt_img = torch.cat([images[None, 0:pi, :], images[None, pi + 1:, :]], 1)
								now_spt_lbl = torch.cat([labels[None, 0:pi, None], labels[None, pi + 1:, None]], 1)
					...
					print('Validate \n 0medulla_f1={:.4} dsc={:.4} tpr={:.4} ppv={:.4}'
						  ...
						  ' \n 8mes_f1={:.4} dsc={:.4} tpr={:.4} ppv={:.4}\n'
						  ' \n 9mes_f1={:.4} dsc={:.4} tpr={:.4} ppv={:.4}\n'
					...
					df.loc[8] = ['8pod', avg_val_F1[8].item(), avg_val_Dice[8].item(), avg_val_TPR[8].item(), avg_val_PPV[8].item()]
					df.loc[9] = ['9mes', avg_val_F1[9].item(), avg_val_Dice[9].item(), avg_val_TPR[9].item(),avg_val_PPV[9].item()]
