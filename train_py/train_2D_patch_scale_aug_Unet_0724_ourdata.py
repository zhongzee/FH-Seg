...
def get_arguments():
    ...
    parser.add_argument("--trainset_dir", type=str, default='./data/omniseg-sampled/train') # 训练集路径
    ...
    parser.add_argument("--valset_dir", type=str, default='./data/omniseg-sampled/val') # 验证集路径
    parser.add_argument("--exname", type=str, default='train_2D_patch_scale_aug_Unet_for_6class') #日志存储路径名
    ...
    parser.add_argument("--train_list", type=str, default='./data/omniseg-sampled/train/data_list.csv') # 训练集列表
    parser.add_argument("--val_list", type=str, default='./data/omniseg-sampled/val/data_list.csv') # 验证集列表
    parser.add_argument("--edge_weight", type=float, default=1.2)

    parser.add_argument("--scale", type=str2bool, default=False)
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/Unet/train_2D_patch_scale_aug_Unet_0724_ourdata/')
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for epoch in range(0,args.num_epochs):
            ...
            for iter, batch in enumerate(trainloader):
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    for bi in range(len(scales)):
                        print("len task0_scale=", len(task0_scale))
                        print("task0_scale=", task0_scale)
            ...
            if (task0_pool_image.num_imgs < batch_size) & (task0_pool_image.num_imgs > 0):
                    ...
                    for bi in range(len(scales)):
                        print("len task0_scale < batchsize =", len(task0_scale))
                        print("task0_scale < batchsize vi =", task0_scale)
                        scales[bi] = task0_scale.pop(0)
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/fold1_with_white',  'experiments/DoDNet/MIDL/Unet/HC_validation'), str(epoch))
                    ...
                    print('Validate \n 0-Lumen_f1={:.4} 0-Lumen_dsc={:.4} 0-Lumen_tpr={:.4} 0-Lumen_ppv={:.4}'
                          ...
                          ##########################################################################
                          ######################################################################
                          .format(avg_val_F1[0].item(), avg_val_Dice[0].item(), avg_val_TPR[0].item(),
                                  avg_val_PPV[0].item(),
                                  avg_val_F1[1].item(), avg_val_Dice[1].item(), avg_val_TPR[1].item(),
                                  avg_val_PPV[1].item(),
                                  avg_val_F1[2].item(), avg_val_Dice[2].item(), avg_val_TPR[2].item(),
                                  avg_val_PPV[2].item(),
                                  avg_val_F1[3].item(), avg_val_Dice[3].item(), avg_val_TPR[3].item(),
                                  avg_val_PPV[3].item(),
                                  avg_val_F1[4].item(), avg_val_Dice[4].item(), avg_val_TPR[4].item(),
                                  avg_val_PPV[4].item(),
                                  avg_val_F1[5].item(), avg_val_Dice[5].item(), avg_val_TPR[5].item(),
                                  avg_val_PPV[5].item()
                                  #############################################################################################
                                  #############################################################################################
                ...
                df.loc[0] = ['0-Lumen', avg_val_F1[0].item(), avg_val_Dice[0].item(), avg_val_TPR[0].item(),
                             avg_val_PPV[0].item()]
                df.loc[1] = ['1-Tunica_intima', avg_val_F1[1].item(), avg_val_Dice[1].item(), avg_val_TPR[1].item(),
                             avg_val_PPV[1].item()]
                df.loc[2] = ['2-Tunica_media', avg_val_F1[2].item(), avg_val_Dice[2].item(), avg_val_TPR[2].item(),
                             avg_val_PPV[2].item()]
                df.loc[3] = ['3-Artery', avg_val_F1[3].item(), avg_val_Dice[3].item(), avg_val_TPR[3].item(),
                             avg_val_PPV[3].item()]
                df.loc[4] = ['4-Artery_wall', avg_val_F1[4].item(), avg_val_Dice[4].item(), avg_val_TPR[4].item(),
                             avg_val_PPV[4].item()]
                df.loc[5] = ['5-Hyline', avg_val_F1[5].item(), avg_val_Dice[5].item(), avg_val_TPR[5].item(),
                             avg_val_PPV[5].item()]
                ################################################################################################################
                ################################################################################################################
