...
# 这个函数使用 argparse 库来创建一个参数解析器，并添加多个参数选项，以便用户可以在命令行中指定这些选项。
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/train_2D_patch_scale_aug_AF_Unet_for_6class_omniseg-sampled_0723/')
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        for epoch in range(0,args.num_epochs):
            ...
            #           f'./experiments/visualize/Learning_rate/Learning_Rate_Decay_Epoch_{epoch}.png',
            #           'Learning Rate')
            #
            ...
            #                    f'./experiments/visualize/Learning_rate/Learning_Rate_Changes_Epoch_{epoch}.xlsx')
            ...
            #           f'./experiments/visualize/Loss/Loss_sum_Epoch_{epoch}.png',
            #           'Train Loss')
            #
            # # Save learning rate data to Excel at the end of each epoch
            # epochs_list = list(range(epoch + 1))  # Create a list of epochs up to the current one
            # save_data_to_excel(epochs_list, all_tr_loss,
            #                    f'./experiments/visualize/Loss/Loss_sum_Changes_Epoch_{epoch}.xlsx')
            #
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        output_folder = os.path.join('./experiments/base/'+args.exname+'/validation_noscale_0724', str(epoch))
                ...
                # 保存整个训练过程的数据到单个CSV文件
                output_folder = './experiments/base/'+args.exname+'/validation_noscale_0724/'
