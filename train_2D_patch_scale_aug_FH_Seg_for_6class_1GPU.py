...
# 这个函数使用 argparse 库来创建一个参数解析器，并添加多个参数选项，以便用户可以在命令行中指定这些选项。
    ...
    parser.add_argument("--exname", type=str, default='train_2D_patch_scale_aug_AF_Unet_for_6class') # 日志存储名字
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/FH_Seg/train_2D_patch_scale_aug_FH_Seg_for_0729_6class_partdata_8GPU/')
    ...
    parser.add_argument("--batch_size", type=int, default=1) # 4->8
...
def main():
    ...
    #     local_rank = dist.get_rank()
        ...
        # if engine.distributed:
        #     trainloader,train_sampler = engine.get_train_loader(train_dataset,batch_size=8)
        #     valloader,val_sampler = engine.get_test_loader(val_dataset,batch_size=8)
            ...
            if (epoch >= 0) and (local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        #output_folder = os.path.join(args.snapshot_dir.replace('snapshots_2D/fold1_with_white', '/Data/DoDNet/MIDL/FH_Seg_2D/validation'), str(epoch))
