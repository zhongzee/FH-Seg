...
class unet2D(nn.Module):
    ...
    def forward(self, input, task_id, scale_id):
        ...
        # print("Input to conv1 device: ", input.device)
        x = self.conv1(input)
        # print("Output from conv1 device: ", x.device)
        ...
        ####################################################################
        # x_cond = torch.zeros((len(scale_encoding), x_feat.shape[1] * 7 * 4, 1, 1)).cuda().float()
        ####################################################################
