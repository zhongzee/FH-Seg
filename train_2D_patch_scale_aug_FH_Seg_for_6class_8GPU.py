...
from unet2D_Dodnet_scale_Attention_fullscale2 import UNet2DAttentionFullscale as UNet2D_scaleAttentionFullscale
...
from engine_distributed import Engine
...
# 这个函数使用 argparse 库来创建一个参数解析器，并添加多个参数选项，以便用户可以在命令行中指定这些选项。
    ...
    parser.add_argument("--trainset_dir", type=str, default='./data/LRV/train') # 训练集路径
    ...
    parser.add_argument("--valset_dir", type=str, default='./data/LRV/val') # 验证集路径
    parser.add_argument("--exname", type=str, default='train_2D_patch_scale_aug_FH_Seg_for_6class_8GPU_16K_0729') # 日志存储名字
    ...
    parser.add_argument("--train_list", type=str, default='./data/LRV/train/data_list.csv') # 训练集列表
    parser.add_argument("--val_list", type=str, default='./data/LRV/val/data_list.csv') # 验证集列表
    parser.add_argument("--edge_weight", type=float, default=1.2)
    parser.add_argument("--scale", type=str2bool, default=False)
    parser.add_argument("--snapshot_dir", type=str, default='snapshots_2D/FH_Seg/train_2D_patch_scale_aug_FH_Seg_for_6class_8GPU_16K_0729/')
    ...
    parser.add_argument("--batch_size", type=int, default=8) # 4->8
    parser.add_argument("--nproc_per_node", type=int, default=8)
    parser.add_argument("--master_addr", type=str, default='')
    parser.add_argument("--master_port", type=str, default='')
    parser.add_argument("--world_nums", type=str, default='1')
    parser.add_argument("--nnodes", type=int, default=1)
    parser.add_argument("--num_gpus", type=int, default=8)
    parser.add_argument('--local_rank', type=int, default=0)
    parser.add_argument("--FP16", type=str2bool, default=False)
    parser.add_argument("--num_epochs", type=int, default=200)  # 最大epoch数目
...
def mask_to_box(tensor):
    ...
    # cmin = np.zeros((4))
    # cmax = np.zeros((4))
    batch_size = tensor.shape[0]  # 动态获取batch_size
    rmin = np.zeros((batch_size))
    rmax = np.zeros((batch_size))
    cmin = np.zeros((batch_size))
    cmax = np.zeros((batch_size))
...
def setup(rank, world_size,master_port,master_addr):
    # os.environ['MASTER_ADDR'] = master_addr
    os.environ['MASTER_PORT'] = str(master_port)
    rank = int(rank)
    world_size = int(world_size)  # 确保这是一个整数
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        world_size = args.world_nums
        # world_size = int(os.getenv('WORLD_SIZE', '1'))
        setup(rank, world_size,args.master_port,args.master_addr)
    # 获取当前进程的 rank 和 world size
        ...
        safe_makedirs(snapshot_dir)  # 确保目录已创建

        lock_file = f"{snapshot_dir}/.lock"
        try:
            with FileLock(lock_file):
                # 你的同步代码块
                pass
        except Exception as e:
            print(f"Error acquiring lock on {lock_file}: {e}")
            raise

        try:
            writer = SummaryWriter(snapshot_dir)
        except Exception as e:
            print(f"Error initializing SummaryWriter at {snapshot_dir}: {e}")
            raise
        ###################################################################
        ...
        # model = UNet2D_scale(num_classes=6, weight_std=False)
        model = UNet2D_scaleAttentionFullscale(num_classes=6, weight_std=False)
        ...
        #
        train_dataset = MOTSDataSet_normal(args.trainset_dir, args.train_list, max_iters=args.itrs_each_epoch * args.batch_size,
                        crop_size=input_size, scale=args.random_scale, mirror=args.random_mirror,
                        edge_weight=edge_weight)

        val_dataset = MOTSValDataSet(args.valset_dir, args.val_list, max_iters=args.itrs_each_epoch * args.batch_size,
                           crop_size=input_size, scale=args.random_scale, mirror=args.random_mirror,
                           edge_weight=edge_weight)

        if engine.distributed:
            trainloader,train_sampler = engine.get_train_loader(train_dataset,batch_size=64)
            valloader,val_sampler = engine.get_test_loader(val_dataset,batch_size=64)
        ...
        for epoch in range(args.start_epoch,args.num_epochs):
            ...
            for iter, batch in enumerate(trainloader):
                ...
                if task0_pool_image.num_imgs >= batch_size:
                    ...
                    # print("Device of scales:", scales.device)

                    preds = model(images, torch.ones(batch_size).cuda()*0, scales)
                    ##################################################################################
                    preds = preds[0] # torch.Size([1, 2, 512, 512])
                    ##################################################################################
                ...
                if task2_pool_image.num_imgs >= batch_size:
                    ...
                    #optimizer.zero_grad()
                    preds = model(images, torch.ones(batch_size).cuda()*2, scales) #images.shape torch.Size([1, 3, 512, 512])
                    ##################################################################################
                    preds = preds[0]
                    ##################################################################################
                    labels = one_hot_3D(labels.long()) # torch.Size([1, 2, 512, 512])
                    weight = edge_weight**wts
            ...
            if (epoch >= 0) and (local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                    for iter, batch in enumerate(valloader):
                        ...
                        output_folder = os.path.join('/root/autodl-tmp/Omni-Seg_revision/experiments/FH_Seg/'+args.exname+'/validation_noscale_0729', str(epoch))
            ...
            if (epoch >= args.num_epochs - 1) and (local_rank == 0):
                print('save model ...')
                if args.FP16:
                    checkpoint = {
                        'model': model.state_dict(),
