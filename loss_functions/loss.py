...
class BinaryPrecisionLoss(nn.Module):
    ...
    def forward(self, predict, target):
        ...
        precision_score = tp / (tp + fp + self.smooth)

        precision_loss = 1 - precision_score

        precision_loss_avg = precision_loss[target[:,0]!=-1].sum() / precision_loss[target[:,0]!=-1].shape[0]

        return precision_loss_avg
...
class Precision4MOTS(nn.Module):
    def __init__(self, weight=None, ignore_index=None, num_classes=3, **kwargs):
        ...
        self.ignore_index = ignore_index
        self.num_classes = num_classes
        self.dice = BinaryPrecisionLoss(**self.kwargs)
