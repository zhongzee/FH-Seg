    ...
    def __getitem__(self, index):
        ...
        if task_id >= 6:
            ...
            if image.shape[0] > 3000 or image.shape[1] > 3000:
                image = image[:3000, :3000, :3]
                label = label[:3000, :3000, :3]
                
                
                # # print('aaa')
                # image = resize(image, (3000, 3000, 3))
                # label = resize(label, (3000, 3000, 3))

                # assert (image.shape == (3000, 3000, 3))
                # assert (label.shape == (3000, 3000, 3))
            ...
            # 
            # image = image[length_x:length_x+length,length_y:length_y+length,:]
            # label = label[length_x:length_x+length,length_y:length_y+length,:]

            # assert (image.shape == (2048, 2048, 3))
            # assert (label.shape == (2048, 2048, 3))
...
if __name__ == '__main__':
    ...
    trainset_dir = '/Data2/KI_data_trainingset_mix_patch/data_list.csv'
    ...
    trainloader = DataLoader(
        MOTSDataSet(trainset_dir, train_list, max_iters=itrs_each_epoch * batch_size,
                    crop_size=input_size, scale=random_scale, mirror=random_mirror,
                    edge_weight=edge_weight), batch_size=1, shuffle=True, num_workers=8)
